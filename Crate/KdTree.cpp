...
std::unique_ptr<KdTree::Node> KdTree::BuildRecursive( //рекурсивно делит пространство по медиане 
    ...
    int depth)
{
    ...
    std::nth_element(objectIndices.begin(), midIt, objectIndices.end(), cmp); //ставит медиану на позицию мид
...
void KdTree::Build(const std::vector<BoundingBox>& worldBounds) //построение дерева из массива mstressworldBounds
{
    mRoot.reset();
...
void KdTree::QueryNode(  //обходит дерево, если ААВВ узла не пересекает пирамиду - ретурн, если нет, то спускается в детей
    const std::vector<BoundingBox>& worldBounds,
    const Node* node,
    const BoundingFrustum& frustumViewSpace,
    FXMMATRIX viewWorld,
