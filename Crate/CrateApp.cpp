...
struct RenderItem
{
    ...
    float LodSwitchDist = 35.0f;  
    bool UseBillboard = false;  
    int BillboardTexSrvIndex = -1;
};
...
private:
    ...
    std::vector<RenderItem*> mStressVisibleBillboards;
    std::vector<RenderItem*> mStressVisible3D;
    float mBillboardSwitchDistance = 5.0f;  // Дистанция переключения на билборд
    int mBillboardTexIndex = 0;  // ИНДЕКС ТЕКСТУРЫ БИЛБОРДА В mSrvDescriptorHeap
    KdTree mKdTree;
...
void CrateApp::Draw(const GameTimer& gt)
{
    ...
    // Рисуем 3D объекты (близкие)
    DrawRenderItems(mCommandList.Get(), mStressVisible3D);

    if (!mStressVisibleBillboards.empty())
    {
        // 1. Переключаем контекст на отрисовку билбордов (PSO, Root Sig, Topology)
        mRenderingSystem->BeginBillboardPass(
            mCommandList.Get(),
            DepthStencilView(),
            passAddress);

        // 2. Готовим дескрипторы
        CD3DX12_GPU_DESCRIPTOR_HANDLE billboardTexHandle(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
        billboardTexHandle.Offset(mBillboardTexIndex, mCbvSrvDescriptorSize); // Индекс 2

        UINT objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants));
        auto objectCB = mCurrFrameResource->ObjectCB->Resource();
        auto billboardGeo = mGeometries["BillboardGeo"].get();
        auto vbv = billboardGeo->VertexBufferView();

        // 3. Биндим вершинный буфер ОДИН раз (он одинаковый для всех билбордов)
        mCommandList->IASetVertexBuffers(0, 1, &vbv);

        for (auto* ri : mStressVisibleBillboards)
        {
            // Биндим текстуру в СЛОТ 2 (согласно BuildBillboardRootSignature)
            mCommandList->SetGraphicsRootDescriptorTable(2, billboardTexHandle);

            // Биндим Object CB в СЛОТ 0 (согласно BuildBillboardRootSignature)
            D3D12_GPU_VIRTUAL_ADDRESS objCBAddress = objectCB->GetGPUVirtualAddress() + ri->ObjCBIndex * objCBByteSize;
            mCommandList->SetGraphicsRootConstantBufferView(0, objCBAddress);

            // Рисуем 1 инстанс, 1 вершину. БЕЗ индексов!
...
void CrateApp::CreateBillboardGeometry()
{
    // Создаем геометрию из ОДНОЙ вершины в центре (0,0,0). 
    // World-матрица из ObjectConstants будет отвечать за позицию и масштаб.
    std::vector<Vertex> vertices(1);
    vertices[0].Pos = { 0.0f, 0.0f, 0.0f };
    vertices[0].Normal = { 0.0f, 1.0f, 0.0f };
    vertices[0].TexC = { 0.5f, 0.5f };
    ...
    // Индексов нет, будем использовать DrawInstanced, а не DrawIndexedInstanced
    geo->IndexFormat = DXGI_FORMAT_UNKNOWN;
    ...
    submesh.StartIndexLocation = 0;
    submesh.BaseVertexLocation = 0;
    geo->DrawArgs["billboard"] = submesh;

    mGeometries[geo->Name] = std::move(geo);
    OutputDebugStringA("Billboard geometry (1 vertex) created successfully\n");
}
...
void CrateApp::LoadTextures()
{
    ...
    // === 3. Billboard Texture (PNG) ===
    OutputDebugStringA("[3/3] Loading billboard texture (PNG)...\n");
    auto billboardTex = std::make_unique<Texture>();
    billboardTex->Name = "billboardTex";
    billboardTex->Filename = L"../Textures/billboard.png";  // ЗАМЕНИ НА СВОЙ ПУТЬ

    // Проверяем существование файла
    DWORD fileAttrib = GetFileAttributesW(billboardTex->Filename.c_str());
    if (fileAttrib == INVALID_FILE_ATTRIBUTES)
    {
        OutputDebugStringA("ERROR: File does not exist, trying absolute path...\n");
        billboardTex->Filename = L"C:/kg8_4/Crate/textures/spnza_bricks_a_diff.png";    
    }

    // Загружаем PNG через DirectXTex
    ScratchImage image;
    TexMetadata metadata;
    HRESULT hr = LoadFromWICFile(billboardTex->Filename.c_str(), WIC_FLAGS_NONE, &metadata, image);
    if (FAILED(hr))
    {
        char buf[256];
        sprintf_s(buf, "ERROR: LoadFromWICFile failed (HR=0x%08X)\n", hr);
        OutputDebugStringA(buf);
        return;
    }

    OutputDebugStringA("[3/3] PNG file parsed OK, creating GPU resource...\n");

    // Создаём ресурс в видеопамяти
    D3D12_RESOURCE_DESC textureDesc = {};
    textureDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
    textureDesc.Alignment = 0;
    textureDesc.Width = metadata.width;
    ...
    hr = md3dDevice->CreateCommittedResource(
        ...
        IID_PPV_ARGS(&billboardTex->Resource));

    if (FAILED(hr))
    {
        OutputDebugStringA("ERROR: CreateCommittedResource failed\n");
        return;
    }

    // Создаём upload heap
    const UINT64 uploadBufferSize = GetRequiredIntermediateSize(
        billboardTex->Resource.Get(), 0, static_cast<UINT>(image.GetImageCount()));
    ...
    hr = md3dDevice->CreateCommittedResource(
        ...
        IID_PPV_ARGS(&billboardTex->UploadHeap));

    if (FAILED(hr))
    {
        OutputDebugStringA("ERROR: Upload heap creation failed\n");
        return;
    }
    ...
    CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(
        billboardTex->Resource.Get(),
        D3D12_RESOURCE_STATE_COMMON,
        D3D12_RESOURCE_STATE_COPY_DEST);
    mCommandList->ResourceBarrier(1, &barrier);

    UpdateSubresources(mCommandList.Get(), billboardTex->Resource.Get(),
        billboardTex->UploadHeap.Get(), 0, 0,
    ...
    OutputDebugStringA("[3/3] GPU resource created, data uploaded\n");

    // Сохраняем текстуру (БЕЗ создания SRV!)
    mTextures[billboardTex->Name] = std::move(billboardTex);
    OutputDebugStringA("[3/3] Billboard texture loaded (SRV will be created in BuildDescriptorHeaps)\n");
}
...
void CrateApp::BuildDescriptorHeaps()
{
    ...
    // === НОВОЕ: SRV для billboardTex (индекс 2) ===
    if (mTextures.find("billboardTex") != mTextures.end() && mTextures["billboardTex"]->Resource)
    {
        hDescriptor.Offset(1, mCbvSrvDescriptorSize);
        auto billboardTex = mTextures["billboardTex"]->Resource;
        srvDesc.Format = billboardTex->GetDesc().Format;
        srvDesc.Texture2D.MipLevels = billboardTex->GetDesc().MipLevels;
        md3dDevice->CreateShaderResourceView(billboardTex.Get(), &srvDesc, hDescriptor);
        OutputDebugStringA("Billboard SRV created at index 2\n");
    }
    else
    {
        OutputDebugStringA("WARNING: billboardTex not found or not loaded\n");
    }
...
void CrateApp::UpdateStressVisibility()
{
    ...
    mStressVisible3D.clear();
    mStressVisibleBillboards.clear();
    ...
    {
        // Без отсечения: разделяем по дистанции
        for (auto& ri : mStressRitems)
        {
            XMVECTOR objPos = XMLoadFloat3(reinterpret_cast<const XMFLOAT3*>(&ri->World._41));
            XMVECTOR eyePos = XMLoadFloat3(&mEyePos);
            float dist = XMVectorGetX(XMVector3Length(objPos - eyePos));

            if (dist > 35.0f)
                mStressVisibleBillboards.push_back(ri);
            else
                mStressVisible3D.push_back(ri);
        }

        // Для обратной совместимости
        mStressVisibleRitems = mStressVisible3D;
        return;
    }

    // С отсечением (Frustum/KD-Tree)
    ...
    {
        char buf[256];
        sprintf_s(buf, "Frame %d: 3D=%zu, Billboards=%zu\n",
            frameCount, mStressVisible3D.size(), mStressVisibleBillboards.size());
        OutputDebugStringA(buf);
    }
}
