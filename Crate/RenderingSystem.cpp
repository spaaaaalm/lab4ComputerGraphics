...
void RenderingSystem::BuildBillboardPSO()
{
    auto billboardVS = d3dUtil::CompileShader(L"Shaders\\Billboard.hlsl", nullptr, "VS", "vs_5_0");
    auto billboardGS = d3dUtil::CompileShader(L"Shaders\\Billboard.hlsl", nullptr, "GS", "gs_5_0");
    auto billboardPS = d3dUtil::CompileShader(L"Shaders\\Billboard.hlsl", nullptr, "PS", "ps_5_0");

    D3D12_GRAPHICS_PIPELINE_STATE_DESC psoDesc = {};
    psoDesc.InputLayout = { mInputLayout.data(), (UINT)mInputLayout.size() };
    psoDesc.pRootSignature = mBillboardRootSignature.Get();
    psoDesc.VS = { billboardVS->GetBufferPointer(), billboardVS->GetBufferSize() };
    psoDesc.GS = { billboardGS->GetBufferPointer(), billboardGS->GetBufferSize() };
    psoDesc.PS = { billboardPS->GetBufferPointer(), billboardPS->GetBufferSize() };
    ...
    psoDesc.NumRenderTargets = GBuffer::BufferCount;
    ...
    ThrowIfFailed(mDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&mBillboardPSO)));
}

void RenderingSystem::BeginBillboardPass(
    ID3D12GraphicsCommandList* cmdList,
    D3D12_CPU_DESCRIPTOR_HANDLE dsv,
    D3D12_GPU_VIRTUAL_ADDRESS passCbAddress)
{
    ...
    cmdList->OMSetRenderTargets(GBuffer::BufferCount, rtvs, false, &dsv);
    cmdList->SetPipelineState(mBillboardPSO.Get());
    cmdList->SetGraphicsRootSignature(mBillboardRootSignature.Get());
    cmdList->SetGraphicsRootConstantBufferView(1, passCbAddress);
